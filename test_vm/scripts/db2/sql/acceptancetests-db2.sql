...
CREATE PROCEDURE raise_error_with_params (IN name VARCHAR(200), OUT strlength INTEGER) LANGUAGE SQL BEGIN IF (name = 'xx') THEN SIGNAL SQLSTATE '38123' SET MESSAGE_TEXT = 'text exception'; END IF; SET strlength = LENGTH(name); RETURN; END

CREATE PROCEDURE raise_error_no_params LANGUAGE SQL BEGIN SIGNAL SQLSTATE '38123' SET MESSAGE_TEXT = 'text exception'; RETURN; END
