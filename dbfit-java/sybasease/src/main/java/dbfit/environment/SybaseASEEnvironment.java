...
System.out.println("SybaseASEEnvironment: getAllProcedureParameters: FUNC: found return_value param: " + paramName);
    ...
    private String objectSchemaName(String dbObjectName) {
        String objectPartName = "";
        String[] objnameParts = dbObjectName.split("\\.");
        if (objnameParts.length == 3) {
            objectPartName = objnameParts[1];
        } else {
            if (objnameParts.length == 2) {
                objectPartName = objnameParts[0];
            }
        }
        return objectPartName;
    }

    private String objectSchemaPrefix(String dbObjectName) {
        return objectSchemaName(dbObjectName) + ".";
    }
    ...
    public boolean routineIsFunction(String routineName) throws SQLException {
        String query = "SELECT 1"
                     + "  FROM " + objectDatabasePrefix(routineName) + "dbo.sysobjects o"
                     + " INNER"
                     + "  JOIN " + objectDatabasePrefix(routineName) + "dbo.sysusers u"
                     + "    ON o.uid = u.uid"
                     + " WHERE u.name = " + (objectSchemaPrefix(routineName).equals("") ? "USER" : ("'" + objectSchemaName(routineName)) + "'")
                     + "   AND o.type = 'SF'"
                     + "   AND o.name = ?";
System.out.println("built query: " + query);
        boolean foundFunction = false;
        try (PreparedStatement dc = currentConnection.prepareStatement(query)) {
            dc.setString(1, objectName(routineName));
            ResultSet rs = dc.executeQuery();
            if (rs.next()) {
                foundFunction =true;
            }
