...
public abstract class AbstractDbEnvironment implements DBEnvironment {
    ...
    protected DbStoredProcedureCommandHelper getDbStoredProcedureCommandHelper() {
        return new DbStoredProcedureCommandHelper();
    }

    @Override
    public PreparedStatement buildStoredProcedurePreparedStatement(String procName,
                DbParameterAccessor[] accessors) throws SQLException {
        DbStoredProcedureCommandHelper spHelper = getDbStoredProcedureCommandHelper();

        String callString = spHelper.buildPreparedStatementString(procName, accessors);
        CallableStatement cs = getConnection().prepareCall(callString);
        spHelper.bindParameters(cs, accessors);
