...
public class SybaseIQEnvironment extends SybaseEnvironment {
...
// HAVE WE MISSED THE "DOUBLE" DATA TYPE?
    ...
    public Map<String, DbParameterAccessor> getAllProcedureParameters(
        ...
        try (PreparedStatement dc = currentConnection.prepareStatement(query)) {
            ...
            ResultSet rs = dc.executeQuery();
            while (rs.next()) {
                String paramName = rs.getString("parm_name");
                String paramType = rs.getString("parm_mode").trim();
                String paramDataType = rs.getString("domain_name");
                Direction direction;
                ...
                params.add("out".equals(paramType) ? "" : paramName,
                           direction,
    ...
    public boolean routineIsFunction(String routineName) throws SQLException {
        String qualifiers[] = routineName.split("\\.");
        String query = "SELECT 1"
                     + "  FROM sysprocparms"
                     + " WHERE creator = " + ((qualifiers.length > 1) ? ("'" + qualifiers[qualifiers.length - 2] + "'") : "USER")
                     + "   AND parmmode = 'OUT'"
                     + "   AND procname = '" + qualifiers[qualifiers.length - 1] + "'";
